...
type Glasses struct {
	...
	Status                      uint        `json:"status" v:"in:1,2#状态只能是1(上线)或2(下线)" dc:"设备状态：1-上线，2-下线"`
	StatusStr                   string      `json:"statusStr" dc:"设备状态：1-上线，2-下线"`
	UseStatus                   uint        `json:"useStatus" v:"in:1,2#状态只能是1(可用)或2(不可用)" dc:"设备使用状态：1-可用，2-不可用"`
	UseStatusStr                string      `json:"useStatusStr" dc:"设备使用状态：1-可用，2-不可用"`
...
type GlassesListReq struct {
	PaginateReq
	Name      string `json:"name"`
	UseStatus int    `json:"useStatus"`
}

type GlassesListRes struct {
	PaginateRes
	Data []*Glasses `json:"data"`
}

type GlassesInfoRes struct {
	TotalCount    int `json:"totalCount" dc:"总设备数"`
